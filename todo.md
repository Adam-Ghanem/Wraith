...
- [ ] After R7 is completed and pushed, create `feature/r7.5-content-discovery` and implement bounded wordlist-based content discovery through R1 and R3 only, with no main changes and no R8 work.
- [ ] After R7 and R7.5 are completed and pushed, create `feature/r8-security-validation` and implement evidence-led, reproducible, R1/R3-only security validation without credential attacks, destructive testing, alternate transport, or main changes.
- [ ] After the requested R8/R9 sequence and any separately confirmed main integration, create `feature/r10-authenticated-security` and implement only R1/R3-controlled, secret-redacted authenticated testing behind explicit operator gates; reconcile the conflicting main-merge instruction before any R10 branch or merge action, and do not start R11.
