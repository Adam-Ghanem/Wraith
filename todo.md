...
- [x] Create `feature/r13.2-assessment-execution` from R13.1; audit existing R13/R13.1/R10.5 execution, persistence, and CLI seams; implement the fail-closed active-assessment execution lifecycle using the existing adapter registry and shared controls; add state, validation, cancellation, persistence, CLI, localhost-only integration, fuzz/benchmark, documentation, and security-review coverage; commit and push only this branch. Do not merge `main` or start R14.
- [x] Create `feature/r14-campaign-orchestration` from R13.2; audit existing R1/R2/R3/R10.5/R11.6/R13/R13.1/R13.2 campaign seams; implement only a bounded project-scoped campaign lifecycle, checkpoint/cycle model, R1 revalidation, R13.2 handoff, and existing lifecycle persistence; add CLI, localhost-only integration, fuzz/benchmark, documentation, security review, commit, and push. Do not merge `main` or start R15.
