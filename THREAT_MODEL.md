...
| Assessment orchestration bypass | A plan executes outside scope, a task bypasses shared budgets/rates, an expired authorization continues, or raw signals become findings directly. | R13 requires explicit authorized scope snapshots with expiry, validates project-scoped dependency graphs, requires injected R10.5 controls, has no transport primitive, and delegates validation/correlation/finding/risk/graph ownership to existing R11 components. |
