...
**Status:** Architecture threat model for the CLI-first Web and API toolkit evolution. It complements the implemented-feature model in [`docs/threat-model.md`](docs/threat-model.md). R1–R10.5 are merged to `main`; R11–R15 remain separately implemented feature-branch slices. R15 reuses the established R3 transport and existing R4/R5/R11.2 owners without adding a client, resolver, scanner, scheduler, or evidence model.
...
| Assessment execution bypass, stale authorization, or secret-bearing lifecycle data | A validated plan runs after scope revocation, a waiting task reaches an adapter without a fresh authorization decision, budget exhaustion races a dispatch, a context/result contains a credential-like value, or a raw signal becomes a finding. | R13.2 validates plan/project/profile/dependencies/registered owner before dispatch; rechecks R1 before execution, scheduling, and dispatch; consumes shared R10.5 controls; derives cancellation-aware global/per-task contexts; rejects secret-like context/results; emits project-scoped bounded events; and reuses R10.5 `{}`-metadata lifecycle storage. The engine owns no transport and does not create findings. |
| Campaign replay, cross-project checkpoint use, stale cycle authorization, or an implied autonomous worker | A durable campaign replays completed tasks, reads another project’s snapshot/checkpoint, treats changed discovery as approval, runs on stale scope, or claims a local status mutation can stop/resume live owner traffic. | R14 persists all campaign state under project-scoped keys, validates deterministic checkpoint fingerprints, filters only explicitly eligible non-completed tasks, rechecks R1 on explicit creation/run and relies on R13.2 pre-dispatch rechecks, writes empty event metadata, and has no worker/scheduler/transport. Pause/cancel/resume scheduling remain deferred rather than misrepresented. |
| Built-in adapter bypass, nested-control deadlock, or implied owner coverage | A campaign adapter constructs alternate egress, reserves a task-wide R10.5 lease that deadlocks delegated owner requests, accepts a credential-bearing context, or reports a missing owner as completed work. | R15 binds only existing R4/R5/R11.2 seams through R13.1. Every active request continues through injected R3; R4 and R11.2 owners acquire the shared R10.5 controls per request; R13.1 rejects credential-bearing targets and invalid limits; incomplete owners return typed `ADAPTER_UNAVAILABLE`; and dry-run dispatches no owner or lifecycle write. |
