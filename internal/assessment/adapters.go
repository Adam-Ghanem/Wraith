...
func (registry AdapterRegistry) Dispatch(ctx context.Context, taskContext TaskContext) (AdapterResult, error) {
	...
	invalidTarget := invalidTaskTarget(taskContext.Scope.Target)
	if taskContext.Task.Type == TaskNetworkPortDiscovery {
		invalidTarget = false
	}
	if len(registry.adapters) == 0 || ctx == nil || taskContext.RunContext.Budget == nil || taskContext.RunContext.Concurrency == nil || taskContext.RunContext.Rate == nil || taskContext.AssessmentID == "" || taskContext.AssessmentID != taskContext.Task.AssessmentID || taskContext.Scope.ProjectID == "" || taskContext.Scope.ProjectID != taskContext.Task.ProjectID || taskContext.Scope.Target != taskContext.Task.Target || invalidTarget || !taskContext.Scope.Authorized || taskContext.Scope.ExpiresAt.IsZero() || !taskContext.Scope.ExpiresAt.After(now().UTC()) || !validLimits(taskContext.Scope.Limits) || !knownTaskType(taskContext.Task.Type) {
