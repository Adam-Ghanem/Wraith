...
func TestEvidenceVerifyUsesExactProjectLocalRecords(t *testing.T) {
	...
	if err := database.AppendObservation(ctx, evidence.Observation{ID: "observation-1", ProjectID: "alpha", Kind: evidence.ObservationKindValidation, SubjectIdentity: "endpoint-1", Source: "test", ObservedAt: now.Add(-2 * time.Hour), Payload: []byte(`{}`), Redacted: true}); err != nil {
		t.Fatal(err)
	}
	if err := database.UpsertSecurityFinding(ctx, storage.SecurityFindingRecord{FindingID: "finding-1", ProjectID: "alpha", RunID: "run-1", ValidationID: "validation-1", CorrelationID: "correlation-1", EndpointID: "endpoint-1", ParameterID: "parameter-1", AssetID: "asset-1", Class: "sql", Subtype: "validated_sql", Title: "Validated behavior", Description: "Bounded evidence", RemediationHint: "Review", Confidence: "high", Severity: "high", RiskScore: 70, RiskBand: "high", RiskModelVersion: "r11.5-v1", RiskFactorsJSON: `[]`, RiskReason: "deterministic", Status: "open", FirstSeenAt: now, LastSeenAt: now, ValidatedAt: now.Add(-time.Hour), RiskCalculatedAt: now, Fingerprint: "finding-fingerprint", EvidenceReferences: []string{"observation-1"}}); err != nil {
		t.Fatal(err)
	}
