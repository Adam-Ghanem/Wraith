...
func TestR6MigrationPreservesR5EvidenceObservations(t *testing.T) {
...
SELECT id, project_id, kind, subject_identity, source, observed_at, payload_json, redacted FROM evidence_observations_r6;
...
INSERT INTO evidence_observations(id, project_id, kind, subject_identity, source, observed_at, payload_json, redacted)
VALUES ('r5-record', 'project-a', 'javascript', 'javascript:https://example.test/app.js', 'crawler', '2026-08-18T00:00:00Z', '{}', 0);`); err != nil {
		t.Fatalf("simulate R5 observation table: %v", err)
	}
	if err := database.Migrate(ctx); err != nil {
		t.Fatalf("migrate R5 observations to R6: %v", err)
	}
	observations, err := database.ListObservations(ctx, "project-a", "javascript:https://example.test/app.js")
	if err != nil || len(observations) != 1 || observations[0].ID != "r5-record" || observations[0].Kind != "javascript" {
