...
SELECT id, project_id, kind, subject_identity, source, observed_at, payload_json, redacted FROM evidence_observations_r9;
...
INSERT INTO evidence_observations(id, project_id, kind, subject_identity, source, observed_at, payload_json, redacted)
VALUES ('r75-record','project-a','content_discovery','GET https://example.test/','content-discovery.r75.result','2026-08-18T00:00:00Z','{}',1);`); err != nil {
		t.Fatal(err)
	}
	if err := db.Migrate(ctx); err != nil {
		t.Fatal(err)
