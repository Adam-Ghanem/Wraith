...
func TestSnapshotProjectsPortStateIntoR18(t *testing.T) {
	...
	if len(snapshot.EndpointIDs) != 1 { t.Fatalf("got %d endpoint identities", len(snapshot.EndpointIDs)) }
	if snapshot.ProjectID != "project-a" || snapshot.ScopeVersion != "scope-v1" { t.Fatalf("unexpected lineage: %#v", snapshot) }
}
...
func TestPortFingerprintIsDeterministic(t *testing.T) {
	...
	if left == "" || left != right { t.Fatalf("fingerprint is not deterministic: %q %q", left, right) }
	if left == PortFingerprint("https://example.invalid/", 443, "tcp", StateClosed) { t.Fatal("state is not bound into fingerprint") }
}
