...
| --- | --- | --- |
| CLI | `discover`, `scan`, `crawl`, `endpoints`, `js`, `fuzz`, `content`, `vhost`, `history`, `export-fixtures`, and `version` dispatch through `internal/cli`. | Active workflows require operator-supplied authorization flags; `fuzz` and R7.5 discovery require explicit project-scoped selections and route every request through R3. |
