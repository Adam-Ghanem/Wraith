...
| T9 release trust (feature branch) | `internal/releasetrust`, `wraith release`, canonical manifests/provenance, explicit local Ed25519 trust roots, and bounded local artifact verification. | Offline only; no key retrieval, network/DNS/socket/process path, signing service, private-key persistence, automated publishing, or replacement of T1–T8 authorities. |
